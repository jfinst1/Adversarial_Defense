# Filename: adversarial_defense_system_v8_gpt.py
...
from openai import AsyncOpenAI  # For async ChatGPT calls
...
# ==========================
# ChatGPT Assistant Class
# ==========================
class ChatGPTAssistant:
    def __init__(self, api_key: str = os.getenv('OPENAI_API_KEY'), model_name: str = "gpt-3.5-turbo", temperature: float = 0.0):
        """
        Initialize the ChatGPTAssistant with OpenAI API key and model name.

        Args:
            api_key (str): OpenAI API key from environment variable or direct input.
            model_name (str): The ChatGPT model to use (e.g., gpt-3.5-turbo, gpt-4).
    ...
    async def analyze_input(self, input_text: str) -> str:
        """Analyze input text for adversarial patterns using ChatGPT."""
        prompt = (
    ...
    async def explain_prediction(self, input_text: str, prediction: str) -> str:
        """Generate an explanation for a prediction using ChatGPT."""
        prompt = (
    ...
    async def generate_adversarial_example(self, input_text: str) -> str:
        """Generate an adversarial example using ChatGPT."""
        prompt = (
    ...
    async def _make_api_call(self, prompt: str, temperature: Optional[float] = None) -> str:
        """Helper method for async API calls to ChatGPT."""
        try:
            ...
            logger.error(f"Error during ChatGPT API call: {e}")
...
class AdversarialDefenseSystem:
    def __init__(self, api_key: str = os.getenv('OPENAI_API_KEY'),
                 model_name: str = os.getenv('GPT_MODEL_NAME', 'gpt-3.5-turbo'),
        ...
        self.llm_assistant = ChatGPTAssistant(api_key=api_key, model_name=model_name)
    ...
    async def sanitize_and_detect(self, input_text: str, input_vector: np.ndarray, lightweight: bool = False):
        ...
        logger.info(f"ChatGPT: {ollama_analysis}")
